// I copied sui implementation to add `is_empty` & `borrow` & `length` methods
// If this were a real task, I would do PR in SUI repo

// NOTE: I would also try to implement this type without
//       using drop, so that in the orderbook mod the 
//       balances are stored directly in `Offer` 
//       instead of separately in `Pair`
module orderbook::priority_queue {
    use std::vector;
    ...
    public fun borrow<T: drop>(pq: &PriorityQueue<T>, index: u64): (&u64, &T) {
        let borrow = vector::borrow(&pq.entries, index);
        (&borrow.priority, &borrow.value)
    }
